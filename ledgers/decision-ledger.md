...
| D-042 | A conformance test must fail closed on a missing value; an exemption is an explicit enumerated state, never an absence | The §9.2 check carried `rv is not None` and so exempted 6 of 11 records ([`FTRO-DEF-034`](deficiency-log.md#ftro-def-034)) | 05 |
| D-043 | Every curated record a generator can produce is reconciled against that generator by test | `pin_ppta.py` emitted four identities that differed from the manifest and nothing compared them ([`FTRO-DEF-035`](deficiency-log.md#ftro-def-035)) | 05 |
...
| D-024 | Content-shape validation added to FTRO retrieval tooling | DEF-018 was logged against our own tooling in session 01 but not fixed; a self-directed deficiency that is never closed is decoration | 02 |
| D-024a | **Corrected 05:** D-024 originally said "with a live regression test against CDDIS". No such committed test existed. The committed suite uses deterministic fixtures and makes no network call | A decision that describes a test which does not exist is the same defect as a finding that does | 05 |
